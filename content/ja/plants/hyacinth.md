---
id: hyacinth
name: ヒヤシンス
slug: hyacinth
scientific_name: Hyacinthus orientalis
aliases:
  - ガーデンヒヤシンス
  - ダッチヒヤシンス
categories:
  - houseplants
tags:
  - flowering
  - bulb
  - fragrant
  - toxic
safety:
  dogs:
    status: toxic
    severity: moderate
    summary: 球根は強い刺激を与え、嘔吐や下痢を引き起こすことがあります。
  cats:
    status: toxic
    severity: moderate
    summary: どの部分を噛んでも、よだれ、嘔吐、下痢を引き起こすことがあります。
symptoms:
  - 嘔吐
  - 下痢
  - 過度のよだれ
  - 口腔刺激
  - 飲み込み困難
what_to_do: 口を洗い、水を与えてください。嘔吐が続く場合や球根を飲み込んだ場合は、獣医師に連絡してください。
alternatives:
  - phalaenopsis-orchid
  - gloxinia
  - african-violet
sources:
  - name: ASPCA
    url: >-
      https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/hyacinth
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/poison/hyacinth/'
  - name: MSD Veterinary Manual — 中毒学
    url: 'https://www.msdvetmanual.com/toxicology'
  - name: ASPCA — 猫向け有毒・無毒植物リスト
    url: 'https://www.aspca.org/pet-care/animal-poison-control/cats-plant-list'
  - name: ASPCA — 犬向け有毒・無毒植物リスト
    url: 'https://www.aspca.org/pet-care/animal-poison-control/dogs-plant-list'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: ヒヤシンスとペットの安全 — 香りは良いが毒性あり | PetPilot
meta_description: ヒヤシンスはペットの口や胃を刺激することがあります。症状、ケア、安全な花の代替品について学びましょう。
related_plants:
  - tulip
  - daffodil
---
# ヒヤシンス

ヒヤシンスは、花の密生した穂をつける香り豊かな春の球根植物です。シュウ酸カルシウム結晶やその他の刺激物を含んでおり、口の中に強い痛みや胃腸の不調を引き起こします。最も危険な部分は球根です。

ヒヤシンスの葉、花、球根を噛んだペットは、よだれ、嘔吐、下痢を起こすことがあります。球根を扱うことで皮膚刺激も生じることがあります。ヒヤシンスはペットの届かない場所に置き、球根を植えた後は手を洗ってください。
