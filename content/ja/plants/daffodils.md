---
id: daffodils
name: スイセン（水仙）
slug: daffodils
scientific_name: Narcissus spp.
aliases:
  - ナルキッソス
  - ジョンキル
  - スイセン
categories:
  - outdoor-plants
tags:
  - bulb
  - flowering
  - spring
  - toxic
safety:
  dogs:
    status: toxic
    severity: moderate
    summary: 球根が最も毒性が強く、犬では重度の嘔吐や下痢を引き起こすことがあります。
  cats:
    status: toxic
    severity: moderate
    summary: 猫が葉、花、球根をかじると、よだれ、嘔吐、下痢を引き起こすことがあります。
symptoms:
  - 嘔吐
  - 下痢
  - よだれ
  - 腹痛
  - 震え
  - 低血圧
  - 不整脈
what_to_do: ペットが球根を食べた場合、または重度の嘔吐が見られる場合は、獣医師に連絡してください。大量に摂取した場合は、緊急の対症療法が必要になることがあります。
alternatives:
  - tulips
  - roses
  - sunflowers
  - snapdragons
sources:
  - name: ASPCA
    url: >-
      https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/daffodil
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/poison/daffodil/'
  - name: MSD Veterinary Manual — 毒物学
    url: 'https://www.msdvetmanual.com/toxicology'
  - name: ASPCA — 猫用の有毒・無毒植物リスト
    url: 'https://www.aspca.org/pet-care/animal-poison-control/cats-plant-list'
  - name: ASPCA — 犬用の有毒・無毒植物リスト
    url: 'https://www.aspca.org/pet-care/animal-poison-control/dogs-plant-list'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: ペットのスイセン中毒
meta_description: スイセンは犬や猫を病気にする可能性があり、特に球根が危険です。症状、対処法、より安全な春の花の代替品をご覧ください。
related_plants:
  - tulips
symptoms_severity:
  - symptom: 嘔吐
    severity: moderate
  - symptom: 不整脈
    severity: high
notes_for_puppies: 好奇心旺盛な子犬は春の球根を掘り起こすことがありますので、スイセンの花壇に立ち入らせないようにしてください。
---
# スイセン

スイセンは、鮮やかな黄色の花で春の訪れを告げますが、ペットに有毒なアルカロイドを含んでいます。最も危険なのは球根ですが、地上部も口内の刺激や胃腸障害を引き起こすことがあります。
