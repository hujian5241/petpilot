---
id: grass
name: 草
slug: grass
scientific_name: Poaceae family
aliases:
  - 芝生
  - 芝
  - 牧草
categories:
  - outdoor-plants
tags:
  - lawn
  - safe
  - ground-cover
  - grass
safety:
  dogs:
    status: safe
    summary: 毒性はなく、多くの犬が草をかじりますが、食後に吐いてしまうことがよくあります。
  cats:
    status: safe
    summary: 安全です。室内用や屋外の草を猫が食べることは一般的です。
symptoms:
  - 嘔吐
  - 軽い胃腸の不快感
what_to_do: 通常は何もする必要はありません。嘔吐が頻繁に続く場合、または草に薬剤が散布されていた場合は、獣医師に相談してください。
alternatives:
  - white-clover
  - dandelion
  - common-plantain
sources:
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control
  - name: American Kennel Club
    url: https://www.akc.org/expert-advice/health/
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: 草・芝生とペットの安全ガイド | PetPilot
meta_description: 普通の草は犬や猫にとって安全です。ペットが草を食べる理由や、芝生の薬剤が問題になるタイミングを解説します。
---

# 草・芝生

普通の草は犬や猫にとって有毒ではありません。多くのペットは時々草を食べますが、これは胃の不快感を和らげたり、食物繊維を補ったりするためだと考えられています。食後すぐに嘔吐を起こすことがよくあります。

より大きな懸念は、薬剤処理、肥料、そして寄生虫です。ペットに安全な芝生用製品を使用し、薬剤が散布されているかわからない芝生で草を食べる行為は避けさせるようにしましょう。