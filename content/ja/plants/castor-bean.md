---
id: castor-bean
name: トウゴマ
slug: castor-bean
scientific_name: Ricinus communis
aliases:
  - ヒマシ油植物
  - リキヌス
categories:
  - outdoor-plants
tags:
  - shrub
  - tropical
  - toxic
  - ornamental
safety:
  dogs:
    status: toxic
    severity: critical
    summary: 種子には犬にとって致命的な強力な毒素であるリシンが含まれています。
  cats:
    status: toxic
    severity: critical
    summary: 猫にとって極めて毒性が高く、誤飲すると重篤な腹痛、けいれん、死に至ることがあります。
symptoms:
  - よだれ
  - 腹痛
  - 嘔吐
  - 下痢
  - 脱力
  - 振戦
  - けいれん
  - 昏睡
what_to_do: すぐに救急対応が可能な動物病院へ向かってください。トウゴマ中毒は致命的になることがあり、緊急治療が必要です。
alternatives:
  - sunflowers
  - roses
  - snapdragons
  - zinnias
sources:
  - name: ASPCA
    url: >-
      https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/castor-bean
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/poison/castor-bean/'
  - name: MSD Veterinary Manual — 毒物学
    url: 'https://www.msdvetmanual.com/toxicology'
  - name: ASPCA — 猫向けの有毒・無毒植物リスト
    url: 'https://www.aspca.org/pet-care/animal-poison-control/cats-plant-list'
  - name: ASPCA — 犬向けの有毒・無毒植物リスト
    url: 'https://www.aspca.org/pet-care/animal-poison-control/dogs-plant-list'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: true
meta_title: ペットにおけるトウゴマ中毒
meta_description: トウゴマの種子にはリシンが含まれ、ペットにとって致命的です。緊急時の対処法と、庭に植える安全な観賞用植物の代替品をご紹介します。
symptoms_severity:
  - symptom: けいれん
    severity: critical
  - symptom: 昏睡
    severity: critical
  - symptom: 腹痛
    severity: high
notes_for_puppies: トウゴマの種子は大きく、噛みやすく見えることがあります。ペットがいる場所からはこの植物を取り除いてください。
---
# トウゴマ

トウゴマは、大きなトロピカルな葉とトゲのある種子鞘を持つ植物として知られています。種子には、植物毒素の中でも最も強力なリシンが含まれています。数粒を噛んだだけでも、犬や猫にとって命に関わる危険があります。
