---
id: zucchini
name: Zucchini
slug: zucchini
aliases:
  - courgette
categories:
  - vegetables
tags:
  - low-calorie
  - fiber
  - summer-squash
safety:
  dogs:
    status: safe
    summary: Safe and easy to digest. A good low-calorie filler.
  cats:
    status: safe
    summary: Safe in small amounts when plain and cooked.
symptoms: []
what_to_do: No action needed for plain zucchini.
alternatives:
  - cucumber
  - green-beans
  - carrots
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Steam, bake, or serve raw in small pieces. Do not add butter, salt, or spices.
safe_amount: A few tablespoons for dogs; a teaspoon for cats.
frequency: A few times per week.
related_foods:
  - cucumber
condition_warnings: []
meta_title: Can Dogs Eat Zucchini? Safety, Risks & Vet Advice
meta_description: Is Zucchini safe for dogs and cats? Zucchini is safe for dogs and safe for cats. Learn symptoms, what to do, and vet-approved alternatives.
---

# Zucchini

Zucchini is a mild, easy-to-digest vegetable that is safe for most dogs. It is low in calories and can be added to meals as a filler for overweight pets. Raw zucchini is safe, but cooking it makes it softer and easier to digest.

Cats can have a small amount of plain cooked zucchini. Avoid zucchini bread or dishes containing garlic, onions, or cheese. Always introduce new vegetables gradually to watch for digestive upset.
