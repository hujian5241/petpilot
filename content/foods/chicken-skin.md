...
safety:
  dogs:
    status: limited
    severity: low
    summary: High in fat and often seasoned. Avoid or feed only tiny plain amounts.
  cats:
    status: limited
    severity: low
    summary: High in fat and often seasoned. Avoid or feed only tiny plain amounts.
...
condition_warnings:
  - condition: Obesity
    ...
    reason: High in fat and calories; the file states it is not a healthy treat.
  ...
  - condition: Heart disease/hypertension
    appliesTo:
      ...
      - cats
    recommendation: avoid
    reason: Often seasoned with salt and spices that are unsafe for cardiac patients.
