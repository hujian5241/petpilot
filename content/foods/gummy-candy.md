---
id: gummy-candy
name: Gummy Candy
slug: gummy-candy
aliases:
  - gummy-bears
  - fruit-snacks
categories:
  - sweets
  - human-foods
tags:
  - sugar
  - choking-risk
safety:
  dogs:
    status: limited
    severity: low
    summary: Not toxic, but high sugar and choking risk.
  cats:
    status: limited
    severity: low
    summary: High sugar and can stick to teeth.
symptoms:
  - upset-stomach
  - vomiting
  - diarrhea
  - choking
what_to_do: Contact your vet if sugar-free gummies or large amounts were eaten.
alternatives:
  - blueberries
  - strawberries
  - carrots
sources:
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control
  - name: Cortinovis & Caloni, Front Vet Sci (2016)
    url: https://pubmed.ncbi.nlm.nih.gov/27047944/
  - name: Gugler et al., Compend Contin Educ Vet (2013)
    url: https://pubmed.ncbi.nlm.nih.gov/23677840/
  - name: MSD Veterinary Manual — Xylitol Toxicosis in Dogs
    url: https://www.msdvetmanual.com/toxicology/food-hazards/xylitol-toxicosis-in-dogs
  - name: Murphy & Dunayer, Vet Clin North Am Small Anim Pract (2012) — Xylitol toxicosis in dogs
    url: https://pubmed.ncbi.nlm.nih.gov/22381181/
  - name: 'Murphy & Dunayer, Vet Clin North Am Small Anim Pract (2018) — Xylitol Toxicosis in Dogs: An Update'
    url: https://pubmed.ncbi.nlm.nih.gov/30064708/
  - name: Piscitelli et al., Compend Contin Educ Vet (2010) — Xylitol toxicity in dogs
    url: https://pubmed.ncbi.nlm.nih.gov/20473849/
  - name: ASPCA — People Foods to Avoid Feeding Your Pets
    url: https://www.aspca.org/pet-care/aspca-poison-control/people-foods-avoid-feeding-your-pets
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Avoid gummy candy. Some contain xylitol.
safe_amount: Avoid.
frequency: Avoid.
condition_warnings:
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Concentrated sugar causes blood sugar spikes.
  - condition: Obesity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Empty calories.
  - condition: Dental disease
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Sticky sugar adheres to teeth and promotes decay.
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Sugar alcohols and artificial sweeteners, especially xylitol, can cause severe GI upset.
meta_title: Can Dogs Eat Gummy Candy? Safety, Risks & Vet Advice
meta_description: Is Gummy Candy safe for dogs and cats? Gummy Candy is limited for dogs and limited for cats. Learn symptoms, what to do, and vet-approved alternatives.
---

# Gummy Candy

Gummy candy is high in sugar and can be a choking hazard, especially for small pets. Some sugar-free gummies contain xylitol, which is dangerous to dogs.

It is best not to share gummy candy with pets.
