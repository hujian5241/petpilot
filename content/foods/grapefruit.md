---
id: grapefruit
name: Grapefruit
slug: grapefruit
aliases:
  - grapefruit slices
categories:
  - fruits
tags:
  - limited
  - citrus
safety:
  dogs:
    status: limited
    severity: low
    summary: Not recommended. Very acidic and may interact with medications.
  cats:
    status: limited
    severity: low
    summary: Not recommended.
symptoms:
  - vomiting
  - diarrhea
  - drooling
what_to_do: Contact your vet if a significant amount was consumed, especially if your pet takes medication.
alternatives:
  - oranges
  - blueberries
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Avoid feeding grapefruit.
safe_amount: Avoid.
frequency: Not recommended.
condition_warnings:
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Highly acidic flesh and citrus oils commonly cause vomiting and diarrhea.
  - condition: Dental disease
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Acid can contribute to enamel erosion.
meta_title: Can Dogs Eat Grapefruit? Safety, Risks & Vet Advice
meta_description: Is Grapefruit safe for dogs and cats? Grapefruit is limited for dogs and limited for cats. Learn symptoms, what to do, and vet-approved alternatives.
---

# Grapefruit

Grapefruit is highly acidic and can cause digestive upset. It may also interact with certain medications.
