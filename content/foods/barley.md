---
id: barley
name: Barley
slug: barley
aliases:
  - pearled barley
  - hulled barley
categories:
  - grains
tags:
  - whole-grain
  - fiber
  - chewy
safety:
  dogs:
    status: safe
    summary: Safe when cooked thoroughly. High in fiber.
  cats:
    status: safe
    summary: Safe in very small amounts when fully cooked.
symptoms:
  - gas
  - mild diarrhea
what_to_do: Reduce portion if digestive upset occurs. Do not feed uncooked barley.
alternatives:
  - brown-rice
  - quinoa
  - white-rice
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Cook until soft. Avoid soups or stews with onion, garlic, salt, or fat.
safe_amount: A tablespoon for small dogs, up to a quarter cup for large dogs; a pinch for cats.
frequency: Occasionally.
related_foods:
  - quinoa
  - farro
  - bulgur
condition_warnings:
  - condition: Food allergies
    appliesTo:
      - dogs
      - cats
    recommendation: consult_vet
    reason: Grains/gluten can be allergenic for some pets.
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: High-carbohydrate grain affects blood glucose.
  - condition: Obesity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Calorie-dense carbohydrate source.
  - condition: Inflammatory bowel disease (IBD)
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: High fiber may worsen diarrhea during flare-ups.
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Fiber and gluten can cause gas or loose stools in sensitive pets.
meta_title: Can Dogs Eat Barley? Safety, Risks & Vet Advice
meta_description: Is Barley safe for dogs and cats? Barley is safe for dogs and safe for cats. Learn symptoms, what to do, and vet-approved alternatives.
---

# Barley

Barley is a nutritious, fiber-rich grain that is safe for dogs when fully cooked. It is sometimes used as an ingredient in commercial dog foods. Its chewy texture and high fiber content can be beneficial but may cause gas in sensitive dogs.

Cats can eat small amounts of plain cooked barley but do not require grains. Do not feed barley from soups or stews, which are usually seasoned with onion, garlic, salt, and fats.
