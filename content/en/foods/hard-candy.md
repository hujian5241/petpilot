---
id: hard-candy
name: Hard Candy
slug: hard-candy
aliases:
  - lollipops
  - candy-canes
categories:
  - sweets
  - human-foods
tags:
  - choking-risk
  - sugar
safety:
  dogs:
    status: limited
    severity: low
    summary: 'Not toxic, but poses choking and dental risks.'
  cats:
    status: limited
    severity: low
    summary: Choking hazard and high sugar.
symptoms:
  - choking
  - dental-damage
  - upset-stomach
what_to_do: Contact your vet if your pet choked or ate sugar-free hard candy.
alternatives:
  - blueberries
  - carrots
sources:
  - name: ASPCA
    url: 'https://www.aspca.org/pet-care/animal-poison-control'
  - name: 'Cortinovis & Caloni, Front Vet Sci (2016)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/27047944/'
  - name: 'Gugler et al., Compend Contin Educ Vet (2013)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/23677840/'
  - name: MSD Veterinary Manual — Xylitol Toxicosis in Dogs
    url: >-
      https://www.msdvetmanual.com/toxicology/food-hazards/xylitol-toxicosis-in-dogs
  - name: >-
      Murphy & Dunayer, Vet Clin North Am Small Anim Pract (2012) — Xylitol
      toxicosis in dogs
    url: 'https://pubmed.ncbi.nlm.nih.gov/22381181/'
  - name: >-
      Murphy & Dunayer, Vet Clin North Am Small Anim Pract (2018) — Xylitol
      Toxicosis in Dogs: An Update
    url: 'https://pubmed.ncbi.nlm.nih.gov/30064708/'
  - name: >-
      Piscitelli et al., Compend Contin Educ Vet (2010) — Xylitol toxicity in
      dogs
    url: 'https://pubmed.ncbi.nlm.nih.gov/20473849/'
  - name: ASPCA — People Foods to Avoid Feeding Your Pets
    url: >-
      https://www.aspca.org/pet-care/aspca-poison-control/people-foods-avoid-feeding-your-pets
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Avoid hard candy. Sugar-free versions may contain xylitol.
safe_amount: Avoid.
frequency: Avoid.
condition_warnings:
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Pure sugar causes blood sugar spikes.
  - condition: Obesity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Empty calories.
  - condition: Dental disease
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Hard sugar can fracture teeth and promote decay.
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: >-
      Sugar alcohols and artificial sweeteners can upset the stomach; xylitol is
      toxic to dogs.
meta_title: 'Can Dogs Eat Hard Candy? Safety, Risks & Vet Advice'
meta_description: >-
  Is Hard Candy safe for dogs and cats? Hard Candy is limited for dogs and
  limited for cats. Learn symptoms, what to do, and vet-approved alternatives.
---
# Hard Candy

Hard candy is not typically toxic, but it can cause choking or break teeth. Sugar-free hard candy may contain xylitol, which is extremely toxic to dogs.

Keep hard candies away from pets and offer soft, safe treats instead.
