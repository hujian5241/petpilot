---
id: leeks
name: Leeks
slug: leeks
aliases:
  - leek
categories:
  - vegetables
  - human-foods
tags:
  - allium
  - hemolytic
  - toxic
safety:
  dogs:
    status: toxic
    severity: high
    summary: Toxic. Allium family vegetable that can cause anemia.
  cats:
    status: toxic
    severity: high
    summary: Toxic. Can damage red blood cells.
symptoms:
  - weakness
  - lethargy
  - pale gums
  - red or brown urine
  - vomiting
  - diarrhea
what_to_do: Seek veterinary care immediately after ingestion.
alternatives:
  - carrots
  - green-beans
  - zucchini
sources:
  - name: ASPCA
    url: 'https://www.aspca.org/pet-care/animal-poison-control'
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/'
  - name: 'Cortinovis & Caloni, Front Vet Sci (2016)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/27047944/'
  - name: 'Gugler et al., Compend Contin Educ Vet (2013)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/23677840/'
  - name: MSD Veterinary Manual — Garlic and Onion (Allium spp) Toxicosis
    url: >-
      https://www.msdvetmanual.com/toxicology/food-hazards/garlic-and-onion-allium-spp-toxicosis-in-animals
  - name: >-
      Salgado et al., J Venom Anim Toxins incl Trop Dis (2011) — Allium species
      poisoning in dogs and cats
    url: 'https://dx.doi.org/10.1590/S1678-91992011000100002'
  - name: ASPCA — People Foods to Avoid Feeding Your Pets
    url: >-
      https://www.aspca.org/pet-care/aspca-poison-control/people-foods-avoid-feeding-your-pets
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: true
preparation_notes: Do not feed any part of the leek.
safe_amount: None.
frequency: Never.
related_foods:
  - onions
  - garlic
  - chives
meta_title: 'Can Dogs Eat Leeks? Safety, Risks & Vet Advice'
meta_description: >-
  Is Leeks safe for dogs and cats? Leeks is toxic for dogs and toxic for cats.
  Learn symptoms, what to do, and vet-approved alternatives.
---
# Leeks

Leeks belong to the Allium family and are toxic to dogs and cats. They contain the same compounds that damage red blood cells as onions and garlic. All parts of the leek plant, including the leaves and bulb, are dangerous.

Leeks are common in soups, stews, and casseroles. Do not share these dishes with your pet, even after picking out visible pieces, because the flavoring may remain in the broth.
