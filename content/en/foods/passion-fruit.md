---
id: passion-fruit
name: Passion Fruit
slug: passion-fruit
aliases:
  - passionfruit
categories:
  - fruits
tags:
  - limited
safety:
  dogs:
    status: limited
    severity: low
    summary: Small amounts of pulp are likely safe; seeds may be hard to digest.
  cats:
    status: limited
    severity: low
    summary: Not recommended.
symptoms:
  - upset stomach
  - diarrhea
what_to_do: Monitor for digestive upset.
alternatives:
  - dragon-fruit
  - blueberries
sources:
  - name: American Kennel Club
    url: 'https://www.akc.org/'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Use only a small amount of pulp. Avoid seeds if possible.
safe_amount: A small teaspoon of pulp.
frequency: Rarely.
condition_warnings:
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: consult_vet
    reason: Seeds can be hard to digest and may cause stomach upset.
  - condition: Inflammatory bowel disease (IBD)
    appliesTo:
      - dogs
      - cats
    recommendation: consult_vet
    reason: Seeds and acidity may irritate an inflamed GI tract.
meta_title: 'Can Dogs Eat Passion Fruit? Safety, Risks & Vet Advice'
meta_description: >-
  Is Passion Fruit safe for dogs and cats? Passion Fruit is limited for dogs and
  limited for cats. Learn symptoms, what to do, and vet-approved alternatives.
---
# Passion Fruit

Passion fruit pulp is likely safe in tiny amounts, but the seeds can be hard to digest.
