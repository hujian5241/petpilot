---
id: candy
name: Candy
slug: candy
aliases:
  - sweets
categories:
  - sweets
  - human-foods
tags:
  - high-sugar
  - choking-risk
safety:
  dogs:
    status: limited
    severity: low
    summary: Plain sugary candy is not toxic but can cause stomach upset.
  cats:
    status: limited
    severity: low
    summary: Not recommended due to sugar.
symptoms:
  - upset-stomach
  - vomiting
  - diarrhea
  - weight-gain
  - choking
what_to_do: Contact your vet immediately if sugar-free candy or chocolate was eaten.
alternatives:
  - blueberries
  - carrots
  - apple-slices
sources:
  - name: ASPCA
    url: 'https://www.aspca.org/pet-care/animal-poison-control'
  - name: American Kennel Club
    url: 'https://www.akc.org/'
  - name: 'Cortinovis & Caloni, Front Vet Sci (2016)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/27047944/'
  - name: 'Gugler et al., Compend Contin Educ Vet (2013)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/23677840/'
  - name: MSD Veterinary Manual — Xylitol Toxicosis in Dogs
    url: >-
      https://www.msdvetmanual.com/toxicology/food-hazards/xylitol-toxicosis-in-dogs
  - name: >-
      Murphy & Dunayer, Vet Clin North Am Small Anim Pract (2012) — Xylitol
      toxicosis in dogs
    url: 'https://pubmed.ncbi.nlm.nih.gov/22381181/'
  - name: >-
      Murphy & Dunayer, Vet Clin North Am Small Anim Pract (2018) — Xylitol
      Toxicosis in Dogs: An Update
    url: 'https://pubmed.ncbi.nlm.nih.gov/30064708/'
  - name: >-
      Piscitelli et al., Compend Contin Educ Vet (2010) — Xylitol toxicity in
      dogs
    url: 'https://pubmed.ncbi.nlm.nih.gov/20473849/'
  - name: ASPCA — People Foods to Avoid Feeding Your Pets
    url: >-
      https://www.aspca.org/pet-care/aspca-poison-control/people-foods-avoid-feeding-your-pets
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: 'Avoid all candy, especially sugar-free varieties that may contain xylitol.'
safe_amount: Avoid.
frequency: Avoid.
condition_warnings:
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Pure sugar causes rapid glucose spikes.
  - condition: Obesity
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Empty calories.
  - condition: Dental disease
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Sugary residue promotes decay.
  - condition: Food allergies
    appliesTo:
      - dogs
      - cats
    recommendation: consult_vet
    reason: 'May contain dairy, wheat, or other allergens.'
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: High sugar commonly causes diarrhea or vomiting.
meta_title: 'Can Dogs Eat Candy? Safety, Risks & Vet Advice'
meta_description: >-
  Is Candy safe for dogs and cats? Candy is limited for dogs and limited for
  cats. Learn symptoms, what to do, and vet-approved alternatives.
---
# Candy

Most sugary candy is not toxic to pets, but it is unhealthy and can cause digestive upset. Hard candies can be a choking hazard. Sugar-free candy is especially dangerous because it may contain xylitol.

Keep candy bowls out of reach and offer pet-safe treats instead.
