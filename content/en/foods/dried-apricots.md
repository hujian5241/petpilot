---
id: dried-apricots
name: Dried Apricots
slug: dried-apricots
aliases:
  - dried apricot
categories:
  - fruits
tags:
  - limited
  - high-sugar
safety:
  dogs:
    status: limited
    severity: low
    summary: Safe in very small amounts but high in sugar and fiber. Ensure no pit.
  cats:
    status: limited
    severity: low
    summary: Not recommended due to concentrated sugar.
symptoms:
  - upset stomach
  - diarrhea
  - weight gain
what_to_do: Monitor for digestive upset.
alternatives:
  - apricots
  - apple-slices
sources:
  - name: American Kennel Club
    url: 'https://www.akc.org/'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: 'Choose unsweetened, unsulfured varieties. Ensure pit is removed.'
safe_amount: One small piece.
frequency: Rarely.
condition_warnings:
  - condition: Obesity
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: >-
      Concentrated sugar and calories make dried apricots inappropriate for
      overweight pets.
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Concentrated sugar content can cause blood glucose spikes.
  - condition: Dental disease
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: 'Sticky, sugary dried fruit adheres to teeth and promotes dental disease.'
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: High fiber and sugar can cause upset stomach or diarrhea.
meta_title: 'Can Dogs Eat Dried Apricots? Safety, Risks & Vet Advice'
meta_description: >-
  Is Dried Apricots safe for dogs and cats? Dried Apricots is limited for dogs
  and limited for cats. Learn symptoms, what to do, and vet-approved
  alternatives.
---
# Dried Apricots

Dried apricots are safe in tiny amounts but are concentrated in sugar and fiber, which can cause digestive upset.
