---
id: chives
name: Chives
slug: chives
aliases:
  - green chives
  - chopped chives
categories:
  - vegetables
  - spices
  - human-foods
tags:
  - allium
  - hemolytic
  - toxic
safety:
  dogs:
    status: toxic
    severity: high
    summary: Toxic. Member of the Allium family. Can damage red blood cells.
  cats:
    status: toxic
    severity: high
    summary: Toxic. Can cause anemia and digestive upset.
symptoms:
  - weakness
  - lethargy
  - pale gums
  - red or brown urine
  - vomiting
  - diarrhea
what_to_do: Contact a veterinarian immediately if ingested.
alternatives:
  - carrots
  - green-beans
  - cucumber
sources:
  - name: ASPCA
    url: 'https://www.aspca.org/pet-care/animal-poison-control'
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/'
  - name: 'Cortinovis & Caloni, Front Vet Sci (2016)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/27047944/'
  - name: 'Gugler et al., Compend Contin Educ Vet (2013)'
    url: 'https://pubmed.ncbi.nlm.nih.gov/23677840/'
  - name: MSD Veterinary Manual — Garlic and Onion (Allium spp) Toxicosis
    url: >-
      https://www.msdvetmanual.com/toxicology/food-hazards/garlic-and-onion-allium-spp-toxicosis-in-animals
  - name: >-
      Salgado et al., J Venom Anim Toxins incl Trop Dis (2011) — Allium species
      poisoning in dogs and cats
    url: 'https://dx.doi.org/10.1590/S1678-91992011000100002'
  - name: ASPCA — People Foods to Avoid Feeding Your Pets
    url: >-
      https://www.aspca.org/pet-care/aspca-poison-control/people-foods-avoid-feeding-your-pets
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: true
preparation_notes: Do not feed fresh or dried chives.
safe_amount: None.
frequency: Never.
related_foods:
  - onions
  - garlic
  - leeks
meta_title: 'Can Dogs Eat Chives? Safety, Risks & Vet Advice'
meta_description: >-
  Is Chives safe for dogs and cats? Chives is toxic for dogs and toxic for cats.
  Learn symptoms, what to do, and vet-approved alternatives.
---
# Chives

Chives are an Allium herb and are toxic to both dogs and cats. They can damage red blood cells and cause hemolytic anemia. Even small amounts sprinkled on food can be harmful.

Pets that ingest chives may also develop gastrointestinal upset. Avoid feeding any human foods garnished with chives, and remove chives from dishes before sharing even a small bite with your pet.
