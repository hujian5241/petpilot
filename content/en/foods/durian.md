---
id: durian
name: Durian
slug: durian
aliases:
  - king of fruits
categories:
  - fruits
tags:
  - limited
  - high-fat
safety:
  dogs:
    status: limited
    severity: low
    summary: Small amounts of flesh may be safe but high in fat and sugar.
  cats:
    status: limited
    severity: low
    summary: Not recommended.
symptoms:
  - upset stomach
  - diarrhea
  - pancreatitis risk
what_to_do: Monitor for digestive upset. Avoid for overweight pets.
alternatives:
  - bananas
  - blueberries
sources:
  - name: American Kennel Club
    url: 'https://www.akc.org/'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Remove seeds and outer husk.
safe_amount: A tiny piece.
frequency: Rarely.
condition_warnings:
  - condition: Obesity
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Very high in fat and sugar; the file says avoid for overweight pets.
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: High sugar content can disrupt blood glucose control.
  - condition: Pancreatitis
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: >-
      High fat content increases pancreatitis risk; the file lists pancreatitis
      risk as a symptom.
  - condition: Hyperlipidemia
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Very high fat content is inappropriate for hyperlipidemic pets.
  - condition: Gastrointestinal sensitivity
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: High fat and sugar commonly cause upset stomach or diarrhea.
meta_title: 'Can Dogs Eat Durian? Safety, Risks & Vet Advice'
meta_description: >-
  Is Durian safe for dogs and cats? Durian is limited for dogs and limited for
  cats. Learn symptoms, what to do, and vet-approved alternatives.
---
# Durian

Durian is not toxic but is very high in fat and sugar. It should only be offered as a tiny, rare treat.
