---
id: asparagus-fern
name: Asparagus Fern
slug: asparagus-fern
scientific_name: Asparagus setaceus
aliases:
  - lace fern
  - asparagus grass
  - climbing asparagus
categories:
  - houseplants
tags:
  - fern
  - vine
  - toxic
safety:
  dogs:
    status: toxic
    severity: low
    summary: 'Berries and foliage can cause vomiting, diarrhea, and skin irritation.'
  cats:
    status: toxic
    severity: moderate
    summary: 'Can cause vomiting, diarrhea, drooling, and abdominal pain.'
symptoms:
  - vomiting
  - diarrhea
  - abdominal pain
  - excessive drooling
  - skin irritation
what_to_do: >-
  Remove plant access and rinse skin if sap contacted fur. Contact a vet if
  vomiting or diarrhea persists.
alternatives:
  - boston-fern
  - spider-plant
  - baby-tears
sources:
  - name: ASPCA
    url: >-
      https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/asparagus-fern
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/poison/asparagus-fern/'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: Asparagus Fern and Pet Safety — Toxic to Pets | PetPilot
meta_description: >-
  Asparagus ferns can irritate pets' skin and digestive tract. Learn symptoms,
  care, and safe fern alternatives.
---
# Asparagus Fern

Asparagus ferns are delicate, feathery plants often used in hanging baskets. Despite the common name, they are not true ferns and are toxic to pets. The berries that develop on mature plants are especially attractive to animals.

Ingestion can cause vomiting, diarrhea, and abdominal pain. The sap may also irritate skin and cause itching or redness. Boston ferns and spider plants are safer, pet-friendly alternatives with a similar airy appearance.
