---
id: foxglove
name: Foxglove
slug: foxglove
scientific_name: Digitalis purpurea
aliases:
  - digitalis
  - lady's glove
categories:
  - outdoor-plants
tags:
  - flowering
  - biennial
  - toxic
  - cottage-garden
safety:
  dogs:
    status: toxic
    severity: critical
    summary: >-
      Contains cardiac glycosides that can cause severe or fatal heart
      arrhythmias in dogs.
  cats:
    status: toxic
    severity: critical
    summary: >-
      Highly toxic to cats; ingestion can lead to vomiting, weakness, and
      life-threatening cardiac effects.
symptoms:
  - vomiting
  - diarrhea
  - drooling
  - abnormal heart rate
  - weakness
  - confusion
  - seizures
  - collapse
what_to_do: >-
  Treat as an emergency. Go to the vet immediately with a sample of the plant.
  Do not wait for symptoms to appear.
alternatives:
  - snapdragons
  - petunias
  - zinnias
  - cosmos
sources:
  - name: ASPCA
    url: >-
      https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/foxglove
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/poison/foxglove/'
  - name: MSD Veterinary Manual — Toxicology
    url: 'https://www.msdvetmanual.com/toxicology'
  - name: ASPCA — Toxic and Non-Toxic Plant List for Cats
    url: 'https://www.aspca.org/pet-care/animal-poison-control/cats-plant-list'
  - name: ASPCA — Toxic and Non-Toxic Plant List for Dogs
    url: 'https://www.aspca.org/pet-care/animal-poison-control/dogs-plant-list'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: true
meta_title: Foxglove Toxicity in Pets
meta_description: >-
  Foxglove is deadly to dogs and cats. Recognize poisoning symptoms, learn
  emergency care, and discover safe alternatives for your garden.
symptoms_severity:
  - symptom: abnormal heart rate
    severity: critical
  - symptom: collapse
    severity: critical
  - symptom: seizures
    severity: high
notes_for_puppies: >-
  Puppies are drawn to tall flower spikes; remove foxglove from puppy-accessible
  areas.
---
# Foxglove

Foxglove is a striking cottage-garden flower with tall spires of bell-shaped blooms. Despite its beauty, it contains powerful cardiac glycosides similar to those used in human heart medication. In pets, accidental ingestion can quickly become life-threatening.
