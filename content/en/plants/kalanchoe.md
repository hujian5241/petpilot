---
id: kalanchoe
name: Kalanchoe
slug: kalanchoe
scientific_name: Kalanchoe spp.
aliases:
  - flaming katy
  - devil's backbone
  - mother of thousands
categories:
  - houseplants
tags:
  - succulent
  - flowering
  - toxic
safety:
  dogs:
    status: toxic
    severity: moderate
    summary: Can cause vomiting, diarrhea, and heart rhythm abnormalities.
  cats:
    status: toxic
    severity: moderate
    summary: May cause vomiting, diarrhea, and abnormal heart rhythms.
symptoms:
  - vomiting
  - diarrhea
  - excessive drooling
  - abnormal heartbeat
  - weakness
what_to_do: Contact a veterinarian. Seek emergency care if heart rhythm changes, weakness, or collapse occur.
alternatives:
  - haworthia
  - christmas-cactus
  - spider-plant
sources:
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/kalanchoe
  - name: Pet Poison Helpline
    url: https://www.petpoisonhelpline.com/poison/kalanchoe/
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: Kalanchoe and Pet Safety — Toxic Succulent | PetPilot
meta_description: Kalanchoe can upset pets' stomachs and affect heart rhythm. Learn symptoms, care, and safe alternatives.
---

# Kalanchoe

Kalanchoes are succulents that produce clusters of small, colorful flowers. They contain cardiac glycosides that can affect the heart and cause gastrointestinal upset in pets. The Mother of Thousands variety is particularly popular but shares the same risks.

Pets that ingest kalanchoe leaves or flowers may vomit, drool, and develop diarrhea. In severe cases, heart rhythm disturbances can occur. Keep kalanchoes in secure locations away from pets and choose safer succulents such as haworthia.
