---
id: daffodils
name: Daffodils
slug: daffodils
scientific_name: Narcissus spp.
aliases:
  - narcissus
  - jonquil
  - daffodil
  - paperwhite
categories:
  - outdoor-plants
tags:
  - bulb
  - flowering
  - spring
  - toxic
safety:
  dogs:
    status: toxic
    severity: moderate
    summary: >-
      Bulbs are the most toxic part and can cause severe vomiting and diarrhea
      in dogs.
  cats:
    status: toxic
    severity: moderate
    summary: >-
      Chewing leaves, flowers, or bulbs can cause drooling, vomiting, and
      diarrhea in cats.
symptoms:
  - vomiting
  - diarrhea
  - drooling
  - abdominal pain
  - tremors
  - low blood pressure
  - abnormal heart rhythm
what_to_do: >-
  Call your vet if your pet ate bulbs or shows severe vomiting. Large ingestions
  may require emergency supportive care.
alternatives:
  - roses
  - sunflowers
  - zinnias
  - snapdragons
sources:
  - name: ASPCA
    url: >-
      https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/daffodil
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/poison/daffodil/'
  - name: MSD Veterinary Manual — Toxicology
    url: 'https://www.msdvetmanual.com/toxicology'
  - name: ASPCA — Toxic and Non-Toxic Plant List for Cats
    url: 'https://www.aspca.org/pet-care/animal-poison-control/cats-plant-list'
  - name: ASPCA — Toxic and Non-Toxic Plant List for Dogs
    url: 'https://www.aspca.org/pet-care/animal-poison-control/dogs-plant-list'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: Daffodil Poisoning in Pets
meta_description: >-
  Daffodils can make dogs and cats sick, especially the bulbs. Find symptoms,
  treatment tips, and safer spring flower alternatives.
related_plants:
  - tulips
symptoms_severity:
  - symptom: vomiting
    severity: moderate
  - symptom: abnormal heart rhythm
    severity: high
notes_for_puppies: Curious puppies may dig up spring bulbs; block access to daffodil beds.
---
# Daffodils

Daffodils herald spring with cheerful yellow blooms, but they contain alkaloids that are toxic to pets. The bulbs pose the greatest risk, though above-ground parts can also cause oral irritation and gastrointestinal upset.
