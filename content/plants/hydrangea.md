---
id: hydrangea
name: Hydrangea
slug: hydrangea
scientific_name: Hydrangea macrophylla
aliases:
  - hortensia
  - mophead hydrangea
categories:
  - outdoor-plants
tags:
  - shrub
  - flowering
  - toxic
  - shade-tolerant
safety:
  dogs:
    status: toxic
    severity: moderate
    summary: Contains cyanogenic glycosides that can cause vomiting and diarrhea if large amounts are eaten.
  cats:
    status: toxic
    severity: moderate
    summary: Ingestion can lead to vomiting, diarrhea, and depression in cats.
symptoms:
  - vomiting
  - diarrhea
  - drooling
  - abdominal pain
  - lethargy
  - depression
what_to_do: Monitor your pet. Most cases are mild, but contact your vet if vomiting or diarrhea is severe or persistent.
alternatives:
  - roses
  - camellia
  - impatiens
  - hosta
sources:
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/hydrangea
  - name: Pet Poison Helpline
    url: https://www.petpoisonhelpline.com/poison/hydrangea/
  - name: MSD Veterinary Manual — Toxicology
    url: https://www.msdvetmanual.com/toxicology
  - name: ASPCA — Toxic and Non-Toxic Plant List for Cats
    url: https://www.aspca.org/pet-care/animal-poison-control/cats-plant-list
  - name: ASPCA — Toxic and Non-Toxic Plant List for Dogs
    url: https://www.aspca.org/pet-care/animal-poison-control/dogs-plant-list
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: Hydrangea Toxicity in Pets
meta_description: Hydrangeas can cause vomiting and diarrhea in dogs and cats. Learn symptoms, first aid, and pet-safe shrub alternatives for your yard.
symptoms_severity:
  - symptom: vomiting
    severity: moderate
  - symptom: abdominal pain
    severity: moderate
---

# Hydrangea

Hydrangeas are prized for their large, globe-shaped flower clusters, but they are mildly to moderately toxic to pets. The buds and leaves contain cyanogenic glycosides, which can release cyanide when metabolized. Serious poisoning is uncommon but possible with large ingestions.
