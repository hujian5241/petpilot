---
id: camellia
name: Camellia
slug: camellia
scientific_name: Camellia japonica
aliases:
  - japanese camellia
  - rose of winter
categories:
  - outdoor-plants
tags:
  - shrub
  - flowering
  - safe
  - evergreen
safety:
  dogs:
    status: safe
    severity: low
    summary: Non-toxic to dogs. Camellia oil is also used in some pet products.
  cats:
    status: safe
    severity: low
    summary: Non-toxic to cats. A safe ornamental shrub.
symptoms: []
what_to_do: No action needed. Camellias are safe for pets.
alternatives:
  - roses
  - sunflowers
  - snapdragons
  - rosemary-plant
sources:
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control/toxic-and-non-toxic-plants/camellia
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
meta_title: Are Camellias Safe for Pets?
meta_description: Camellias are non-toxic to dogs and cats. Discover why they are a beautiful, pet-safe alternative to toxic flowering shrubs.
---

# Camellia

Camellias are elegant evergreen shrubs with rose-like flowers. They are non-toxic to dogs and cats, making them a safe alternative to toxic flowering shrubs such as azaleas and rhododendrons.
