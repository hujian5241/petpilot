---
id: flour-tortilla
name: Weizen-Tortilla
slug: flour-tortilla
aliases:
  - Weizen-Tortilla
  - Soft-Tortilla
categories:
  - grains
  - human-foods
tags:
  - wheat
  - flatbread
  - high-calorie
safety:
  dogs:
    status: safe
    summary: In kleinen Mengen ungewürzt unbedenklich. Enthält mehr Kalorien als Mais-Tortillas.
  cats:
    status: safe
    summary: In sehr kleinen Mengen ungewürzt unbedenklich.
symptoms:
  - Blähungen
  - Aufgeblähter Bauch
what_to_do: Bei einem kleinen ungewürzten Stück ist kein Handlungsbedarf erforderlich. Vermeiden Sie Wraps und Burritos mit ungeeigneten Füllungen.
alternatives:
  - corn-tortilla
  - plain-bread
  - white-rice
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Ungewürzt servieren. Vermeiden Sie Käse, Fleischfüllungen, Zwiebeln, Knoblauch, Salz und Öl.
safe_amount: Ein kleines Stück für kleine Hunde, eine halbe Tortilla für große Hunde; ein winziges Stück für Katzen.
frequency: Nur gelegentlich.
related_foods:
  - corn-tortilla
  - plain-bread
  - whole-wheat-bread
condition_warnings:
  - condition: Lebensmittelallergien
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Hunde mit Weizenallergie sollten Weizen-Tortillas meiden.
  - condition: Adipositas
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Enthält mehr Kalorien und Fett als Mais-Tortillas.
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Weizenhaltige Kohlenhydrate können die Blutzuckerkontrolle beeinflussen.
  - condition: Gastrointestinale Sensitivität
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Kann Blähungen oder einen aufgeblähten Bauch verursachen.
meta_title: Dürfen Hunde Weizen-Tortilla essen? Sicherheit, Risiken & tierärztlicher Rat
meta_description: Ist