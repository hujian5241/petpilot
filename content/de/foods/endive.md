---
id: endive
name: Endivie
slug: endive
aliases:
  - Escariol
  - Chicorée
categories:
  - vegetables
tags:
  - safe-treat
  - leafy-green
safety:
  dogs:
    status: safe
    severity: low
    summary: In kleinen Mengen unbedenklich. Ein kalorienarmes Blattgemüse.
  cats:
    status: safe
    severity: low
    summary: In sehr kleinen Mengen unbedenklich.
symptoms:
  - Magenverstimmung
  - Durchfall
what_to_do: Beobachten Sie Ihr Tier auf Verdauungsprobleme.
alternatives:
  - green-beans
  - carrots
  - cucumber
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Gründlich waschen und ohne Zusätze servieren. In kleine Stücke schneiden.
safe_amount: Ein paar kleine Blätter für Hunde.
frequency: Gelegentlich als Leckerbissen.
condition_warnings:
  - condition: Verdauungsempfindlichkeit
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Der hohe Ballaststoffgehalt kann Magenverstimmung oder Durchfall verursachen.
  - condition: Chronisch-entzündliche Darmerkrankung (IBD)
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Ballaststoffreiche Nahrung wird bei akuten IBD-Schüben möglicherweise nicht vertragen.
meta_title: 'Dürfen Hunde Endivie essen? Sicherheit, Risiken & tierärztlicher Rat'
meta_description: Ist Endivie für Hunde und Katzen sicher? Endivie ist für Hunde und Katzen unbedenklich. Erfahren Sie mehr zu Symptomen, was zu tun ist und tierärztlich geprüften Alternativen.
---

# Endivie

Endivie ist ein kalorienarmes Blattgemüse, das für Hunde in kleinen Mengen unbedenklich ist. Sie ist zwar kalorienarm, aber ballaststoffreich, daher sollten Sie sie nur in Maß füttern.