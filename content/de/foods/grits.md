---
id: grits
name: Grits
slug: grits
aliases:
  - Maisgrütze
  - Hominy-Grits
categories:
  - grains
tags:
  - corn
  - soft
  - bland
safety:
  dogs:
    status: safe
    summary: Sicher, wenn schlicht gekocht und in Maßen verfüttert.
  cats:
    status: safe
    summary: Sicher in sehr kleinen Mengen, wenn ungewürzt und gekocht.
symptoms:
  - Blähungen
  - leichter Durchfall
what_to_do: Verringern Sie die Portion, wenn Verdauungsbeschwerden auftreten. Achten Sie darauf, dass keine Gewürze enthalten sind.
alternatives:
  - polenta
  - white-rice
  - plain-pasta
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
  - name: ASPCA
    url: https://www.aspca.org/pet-care/animal-poison-control
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Kochen Sie sie weich. Fügen Sie keine Butter, Salz, Käse, Knoblauch oder Garnelen hinzu.
safe_amount: Ein Esslöffel für kleine Hunde, bis zu einer Vierteltasse für große Hunde; ein Teelöffel für Katzen.
frequency: Gelegentlich.
related_foods:
  - polenta
  - corn
  - corn-tortilla
condition_warnings:
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Einfache Kohlenhydrate können den Blutzuckerspiegel erhöhen.
  - condition: Übergewicht
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Kalorienreiches Getreide mit begrenztem Nährwert.
  - condition: Verdauungsempfindlichkeit
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Maisbasierte Kohlenhydrate können bei empfindlichen Tieren Blähungen verursachen.
meta_title: Können Hunde Grits essen? Sicherheit, Risiken und Tierarzt-Ratschläge
meta_description: Sind Grits für Hunde und Katzen sicher? Grits sind für Hunde und Katzen sicher. Erfahren Sie mehr über Symptome, was zu tun ist, und tierarztgeprüfte Alternativen.
---
```

# Grits

Ungewürzte, gekochte Grits sind als gelegentlicher Leckerbissen für Hunde sicher. Sie sind weich und geschmacksneutral, sodass sie leicht zu fressen sind. Wie andere Maisprodukte bestehen Grits hauptsächlich aus Kohlenhydraten und sollten nicht einen großen Teil der Ernährung ausmachen.

Vermeiden Sie Grits, die mit Butter, Salz, Käse, Speck, Knoblauch oder Garnelen zubereitet wurden, da diese Verdauungsbeschwerden oder Giftwirkungen verursachen können. Katzen können eine sehr kleine Menge ungewürzter Grits bekommen, benötigen sie aber nicht.