---
id: starfruit
name: Sternfrucht
slug: starfruit
aliases:
  - Karambola
categories:
  - fruits
tags:
  - limited
safety:
  dogs:
    status: limited
    severity: low
    summary: >-
      Kleine Mengen können für gesunde Hunde verträglich sein; bei Tieren mit
      Nierenproblemen vermeiden.
  cats:
    status: limited
    severity: low
    summary: Nicht empfohlen. Kann die Nieren beeinträchtigen.
symptoms:
  - Nierenprobleme
  - Magen-Darm-Beschwerden
  - neurologische Anzeichen
what_to_do: >-
  Vermeiden Sie es, Sternfrucht an Haustiere mit Nierenproblemen zu verfüttern.
  Kontaktieren Sie Ihren Tierarzt, wenn Symptome auftreten.
alternatives:
  - blueberries
  - apple-slices
sources:
  - name: American Kennel Club
    url: 'https://www.akc.org/'
  - name: Pet Poison Helpline
    url: 'https://www.petpoisonhelpline.com/'
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Bei Nierenerkrankung Ihres Haustiers meiden.
safe_amount: Vermeiden oder nur winzige Mengen.
frequency: Selten oder nie.
symptoms_severity:
  - symptom: Nierenprobleme
    severity: high
condition_warnings:
  - condition: Chronische Nierenerkrankung
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: >-
      Oxalate und nierenbelastende Verbindungen werden bei chronischer
      Nierenerkrankung schlecht vertragen.
  - condition: Diabetes mellitus
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: >-
      Natürliche Zucker können die Blutzuckerkontrolle beeinflussen; die
      Portionsgröße ist entscheidend.
  - condition: Chronisch-entzündliche Darmerkrankung (IBD)
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Kann die Magen-Darm-Schleimhaut bei empfindlichen Tieren reizen.
  - condition: Harnwegserkrankung/Blasensteine
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Oxalate können zur Bildung von Calciumoxalatsteinen beitragen.
  - condition: Magen-Darm-Empfindlichkeit
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Kann einen sensiblen Magen-Darm-Trakt reizen.
  - condition: Calciumoxalatsteine
    appliesTo:
      - dogs
      - cats
    recommendation: avoid
    reason: Der hohe Oxalatgehalt fördert die Calciumoxalat-Urolithiasis.
meta_title: 'Können Hunde Sternfrucht essen? Sicherheit, Risiken & tierärztliche Empfehlung'
meta_description: >-
  Ist Sternfrucht sicher für Hunde und Katzen? Sternfrucht ist für Hunde
  eingeschränkt und für Katzen eingeschränkt. Erfahren Sie Symptome, was zu tun
  ist und tierarztgeprüfte Alternativen.
---
# Sternfrucht

Sternfrucht kann bei Haustieren mit Nierenkrankheit problematisch sein. Bei gesunden Tieren werden winzige Mengen möglicherweise vertragen.
