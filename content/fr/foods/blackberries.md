---
id: blackberries
name: Mûres
slug: blackberries
aliases:
  - mûre
categories:
  - fruits
tags:
  - safe-treat
safety:
  dogs:
    status: safe
    severity: low
    summary: Sûres avec modération. Bonne source de vitamines et de fibres.
  cats:
    status: safe
    severity: low
    summary: Sûres en très petite quantité.
symptoms:
  - maux d'estomac
  - diarrhée
what_to_do: Surveiller l'apparition de maux d'estomac.
alternatives:
  - blueberries
  - raspberries
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Laver soigneusement. Servir fraîches ou congelées.
safe_amount: Quelques mûres pour les petits chiens ; plusieurs pour les grands chiens.
frequency: Friandise occasionnelle.
condition_warnings:
  - condition: Diabète sucré
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Contient des sucres naturels.
meta_title: Les chiens peuvent-ils manger des mûres ? Sécurité, risques et conseils vétérinaires
meta_description: Les mûres sont-elles sûres pour les chiens et les chats ? Les mûres sont sûres pour les chiens et les chats. Découvrez les symptômes