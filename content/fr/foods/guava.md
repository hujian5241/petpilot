---
id: guava
name: Goyave
slug: guava
aliases:
  - goyaves
categories:
  - fruits
tags:
  - safe-treat
safety:
  dogs:
    status: safe
    severity: low
    summary: Sûre en petites quantités. Retirer les graines si possible.
  cats:
    status: safe
    severity: low
    summary: Sûre en très petites quantités.
symptoms:
  - maux d'estomac
  - diarrhée
what_to_do: Surveiller les troubles digestifs.
alternatives:
  - blueberries
  - kiwi
sources:
  - name: American Kennel Club
    url: https://www.akc.org/
vet_reviewed: true
last_reviewed: '2026-07-01'
requires_emergency_visit: false
preparation_notes: Laver et couper en petits morceaux. Retirer les graines si possible.
safe_amount: Quelques petits morceaux.
frequency: En guise de friandise occasionnelle.
condition_warnings:
  - condition: Diabète sucré
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Les sucres naturels peuvent affecter la glycémie.
  - condition: Obésité
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Le sucre du fruit apporte des calories.
  - condition: Sensibilité gastro-intestinale
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Les fibres et les graines peuvent provoquer une diarrhée.
  - condition: Maladie dentaire
    appliesTo:
      - dogs
      - cats
    recommendation: limit
    reason: Les résidus sucrés du fruit peuvent contribuer aux maladies dentaires.
meta_title: Les chiens peuvent-ils manger de la goyave ? Sécurité, risques et conseils vétérinaires
meta_description: La goyave est-elle sûre pour les chiens et les chats ? La goyave est sûre pour les chiens et sûre pour les chats. Découvrez les symptômes, les mesures à prendre et des alternatives approuvées par le vétérinaire.
---

# Goyave

La goyave est sûre pour les chiens en petites quantités. Les graines sont généralement sans danger, mais les retirer réduit le risque d'étouffement.